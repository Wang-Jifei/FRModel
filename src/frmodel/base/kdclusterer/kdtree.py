...
def construct_kdtree(point_list=None, dimensions=None, axis=0, axis_calc=None):
    ...
    left = construct_kdtree(point_list = point_list[:median], dimensions = dimensions, axis = axis_calc(axis))
...
if __name__ == "__main__":
    ...
    root = construct_kdtree([p1, p2, p3, p4, p5, p6, p7, p8, p9])
    print(root.axis)
    print(root.dimensions)
    print(root.data)
    print(root.candidate_centers)
    print(root.wgt_center)
    print(root.count)
    print(root.real_centroid)
    
    # showNetworkxGraph(root)
    centroids = set([MeansInstance(dimensions = 3) for _ in range(5)])
    root.filter(centroids)
    print(root.candidate_centers)